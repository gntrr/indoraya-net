...
@section('content')
    <div class="bg-white mt-5rem shadow">
        <div class="container p-4 text-center">
            <h2>Ubah Peralatan Jaringan</h2>
        </div>
    </div>
    <div class="container p-4">
        <form enctype="multipart/form-data" action="{{ url('/admin/equipment-networks/' . $equipmentNetwork->id) }}"
              method="post">
            @csrf
            @method('PUT')
            <div class="mb-3">
                <label for="name" class="form-label fw-bold">Nama Peralatan</label>
                <input class="form-control @error('name') is-invalid @enderror" name="name" id="name"
                       aria-describedby="name" placeholder="Masukkan Nama Peralatan *"
                       value="{{ $equipmentNetwork->name }}">
                @error('name')
                <small class="text-danger">{{ $message }} </small>
                @enderror
            </div>
            <div class="mb-3">
                <label for="description" class="form-label fw-bold">Deskripsi Peralatan</label>
                <textarea class="form-control @error('description') is-invalid @enderror" name="description"
                          id="description" rows="3"
                          placeholder="Masukkan Deskripsi *">{{ $equipmentNetwork->description }}</textarea>
                @error('description')
                <small class="text-danger">{{ $message }} </small>
                @enderror
            </div>
            <div class="mb-3">
                <label for="price" class="form-label fw-bold">Harga Peralatan</label>
            ...
            <div class="mb-3">
                ...
                <div>
                    <div class="equipment-image">
                        <img src="{{ asset('storage/' . $equipmentNetwork->image) }}" width="100" height="100"
            ...
            <div class="mb-3">
                <label for="link" class="form-label fw-bold">Link Penjualan</label>
                <textarea class="form-control @error('link') is-invalid @enderror" name="link" id="link" rows="2"
                          placeholder="Masukkan Tautan Penjualan *">{{ $equipmentNetwork->linkn }}</textarea>
                @error('link')
                <small class="text-danger">{{ $message }} </small>
                @enderror
            </div>
                ...
                <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content rounded-3 shadow">
                        <div class="modal-body p-4 text-center">
                            <h5 class="mb-3">Anda akan mengubah peralatan jaringan ini?</h5>
...
@push('script')
    ...
    <script>
        ...
        document.querySelector('.equipment-image').onclick = () => {
